...
func TestWriteBufferAttrASCIIPayloadOrdering(t *testing.T) {
	...
	go func() {
		...
		payload := []byte("1234")
		expected := fmt.Sprintf("WRITE cf-ad9361-lpc BUFFER samples %d\r\n", len(payload))
		if line != expected {
			done <- fmt.Errorf("unexpected command line: %q", line)
			return
		}
	...
	status, err := mgr.WriteBufferAttrASCII("cf-ad9361-lpc", "samples", []byte("1234"))
	if err != nil {
		t.Fatalf("WriteBufferAttrASCII returned error: %v", err)
	}
	if status != 0 {
		t.Fatalf("expected status 0, got %d", status)
	}
...
func TestWriteBufferAttrASCIIEmptyPayload(t *testing.T) {
	client, server := net.Pipe()
	defer client.Close()
	defer server.Close()
	...
	go func() {
		...
		expected := "WRITE cf-ad9361-lpc BUFFER cyclic 0\r\n"
		if line != expected {
			done <- fmt.Errorf("unexpected command line: %q", line)
			return
		}
	...
	status, err := mgr.WriteBufferAttrASCII("cf-ad9361-lpc", "cyclic", nil)
	if err != nil {
		t.Fatalf("WriteBufferAttrASCII returned error: %v", err)
	}
	if status != 0 {
...
func TestWriteBufferAttrASCIINegativeStatus(t *testing.T) {
	client, server := net.Pipe()
	defer client.Close()
	defer server.Close()
	...
	status, err := mgr.WriteBufferAttrASCII("cf-ad9361-lpc", "watermark", []byte("payload"))
